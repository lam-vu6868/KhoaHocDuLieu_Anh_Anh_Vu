...
# =====================================================================
#  LỌC SẢN PHẨM & CHUẨN HÓA ĐỊA DANH
# =====================================================================
...
# =====================================================================
#  XỬ LÝ BRAND & GIÁ TIỀN 
# =====================================================================
print("🧹 Đang làm sạch Brand, gán UNKNOWN và xử lý giá tiền...")
...
# 🚨 Gán 'UNKNOWN' cho các dòng vẫn trống
...
# Xử lý bớt số 0 thừa ở data gốc
...
# Thường công thức là Q3 + 1.5*IQR. Nhưng với đồ công nghệ, giá phân tán rộng nên dùng hệ số 3.0 (Extreme Outliers) cho an toàn, tránh xóa nhầm máy Flagship.
upper_bound = Q3 + 3.0 * IQR 

# Tính trung vị
median_price = df_data_clean['original_price'].median()
median_discount = df_data_clean['discount_price'].median()

# THAY THẾ NGOẠI LAI BẰNG TRUNG VỊ THAY VÌ XÓA 
outliers_count = (df_data_clean['original_price'] > upper_bound).sum()
df_data_clean.loc[df_data_clean['original_price'] > upper_bound, 'original_price'] = median_price
df_data_clean.loc[df_data_clean['discount_price'] > upper_bound, 'discount_price'] = median_discount
print(f"✅ Đã thay thế {outliers_count} sản phẩm có giá vượt trần bằng giá trị trung vị.")

# Xử lý discount
df_data_clean['discount'] = pd.to_numeric(df_data_clean['discount'].astype(str).str.replace('%',''), errors='coerce').fillna(0) / 100

# =====================================================================
# GỘP BẢNG (1-1) & XUẤT FILE
# =====================================================================
